...
try:
    ...
    if (not defined("HAVE_POLL") and not defined("USE_WINSOCK") and
        not defined("FD_SETSIZE")):
    ...
    def test_rlimit(keep_open: bool) -> int:
        ...
        if (rl.rlim_cur > 0 and (not hasattr(resource, "RLIM_INFINITY") or
        ...
        while True:
            ...
            if not testfd:
                print("testfd, malloc() failed", file=sys.stderr)
                num_open.rlim_max /= 2;
            if not num_open.rlim_max or testfd: break
        if not testfd:
        ...
        while num_open.rlim_cur < num_open.rlim_max:
            ...
            if testfd[num_open.rlim_cur] < 0:
                ...
                msg = "closing %s file descriptors" % (rlim2str(num_open.rlim_cur))
                print("%s" % msg, file=sys.stderr)
                ...
                msg = rlim2str(num_open.rlim_max)
                print("shrinking array for %s file descriptors" % msg, file=sys.stderr)

                # we don't care if we can't shrink it

                int *tmpfd;
                tmpfd = libc.realloc(testfd, ct.c_size_t(num_open.rlim_max).value * ct.sizeof(testfd[0]))
                if tmpfd:
                    testfd = tmpfd
                del tmpfd
