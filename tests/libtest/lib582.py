...
def update_fd_set(sockets: Sockets, fdset: lcurl.fd_set, max_fd: lcurl.socket_t):
    ...
    #
    for i in range(sockets.count):
        print("@@@ 1 @@@", FD_ISSET(sockets.sockets[i], ct.byref(fdset)))
        FD_SET(sockets.sockets[i], ct.byref(fdset))
        print("@@@ 2 @@@", FD_ISSET(sockets.sockets[i], ct.byref(fdset)))
        fd = sockets.sockets[i]
        if max_fd.value == lcurl.socket_t(-1).value or fd > max_fd.value:
            max_fd.value = fd


def notify_curl(curl: ct.POINTER(lcurl.CURLM),
...
def check_fd_set(curl: ct.POINTER(lcurl.CURLM),
    ...
    for i in range(sockets.count):
        if FD_ISSET(sockets.sockets[i], ct.byref(fdset)):
            notify_curl(curl, sockets.sockets[i], evBitmask, name)


@curl_test_decorator
def test(URL: str, filename: str, user_login: str = None,
    ...
    with hd_src, curl_guard(True, curl, multi) as guard:
        ...
        while not check_for_completion(multi, success):
            ...
            if timer_timeout.tv_sec != -1 and get_microsec_timeout(timer_timeout) == 0:
                ...
                notify_curl(multi, lcurl.CURL_SOCKET_TIMEOUT, 0, "timeout")
