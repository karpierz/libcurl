...
@curl_test_decorator
def test(URL: str, proxy: str = None, text_mode: str = None) -> lcurl.CURLcode:
    ...
    with curl_guard(True, curl) as guard:
        ...
        test_setopt(curl, lcurl.CURLOPT_PROXY_TRANSFER_MODE, 1)
        test_setopt(curl, lcurl.CURLOPT_VERBOSE, 1)
        if text_mode:
            # enable ASCII/text mode
            test_setopt(curl, lcurl.CURLOPT_TRANSFERTEXT, 1)
